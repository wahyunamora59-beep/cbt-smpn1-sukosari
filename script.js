...
// ==================== FREEZE ====================
function freezeScreen(d = null) { if (isFrozen) return; isFrozen = true; const dur = d !== null ? d : freezeDuration; const o = document.getElementById("freezeOverlay"), t = document.getElementById("freezeTimer"); o.style.display = "flex"; let r = dur; const e = () => { const m = Math.floor(r / 60), s = r % 60; t.textContent = `${m.toString().padStart(2, "0")}:${s.toString().padStart(2, "0")}`; if (r <= 0) { clearInterval(freezeInterval); o.style.display = "none"; isFrozen = false; showToast("Layar sudah tidak dibekukan!", "warning", 5000); } r--; }; e(); freezeInterval = setInterval(e, 1000); }
...
document.addEventListener("keydown",e=>{if(!currentUser||ujianSelesai||isFrozen)return;if(e.target.tagName==='INPUT'||e.target.tagName==='TEXTAREA'||e.target.isContentEditable)return;if(e.key==="F11"||e.key==="Escape"||e.key==="PrintScreen"){e.preventDefault();if(e.key==="PrintScreen"){catatPelanggaran('PRINT_SCREEN','Screenshot');freezeScreen();showToast('🚫 Screenshot tidak diizinkan!','error')}return false}if(e.ctrlKey&&['w','t','n','p','c','v','a','x'].includes(e.key.toLowerCase())){e.preventDefault();catatPelanggaran("KEYBOARD",`Ctrl+${e.key.toUpperCase()}`);showToast(`🚫 Ctrl+${e.key.toUpperCase()} dilarang!`,'warning');freezeScreen();return false}});
...
window.addEventListener('blur', () => { if (!currentUser || ujianSelesai || isFrozen || isDragging) return; const n = Date.now(); if (n - lastBlurTime < BLUR_COOLDOWN) return; lastBlurTime = n; blurCount++; catatPelanggaran('WINDOW_BLUR', `Keluar (${blurCount}/${MAX_BLUR})`); freezeScreen(30); document.getElementById('alertOverlay').style.display = 'block'; document.getElementById('alertSound').play(); showToast(`⚠️ Jangan buka aplikasi lain!`, 'error', 5000); if (blurCount >= MAX_BLUR) showModal({ iconType: 'error', title: '⛔ UJIAN DIBATALKAN', message: `${blurCount}x keluar aplikasi.`, buttons: [{ text: 'OK', type: 'primary', onClick: () => selesaiUjian() }] }); setTimeout(() => document.getElementById('alertOverlay').style.display = 'none', 3000); });
...
document.addEventListener('mouseleave', e => { if (!currentUser || ujianSelesai || isFrozen || isDragging) return; if (e.clientY <= 0 || e.clientX <= 0 || e.clientX >= innerWidth || e.clientY >= innerHeight) { const n = Date.now(); if (n - lastBlurTime < BLUR_COOLDOWN) return; lastBlurTime = n; catatPelanggaran('MOUSE_LEAVE', 'Kursor keluar'); showToast('👆 Kursor keluar area!', 'warning', 3000); } });
document.addEventListener('touchstart', e => { if (!currentUser || ujianSelesai || isFrozen || isDragging) return; if (e.touches.length > 3) { catatPelanggaran('MULTI_TOUCH', `${e.touches.length} jari`); showToast(`⚠️ Multi-touch ${e.touches.length} jari!`, 'warning', 3000); } const n = Date.now(); if (n - touchStart < 100) { touchCount++; if (touchCount > 5) { catatPelanggaran('RAPID_TOUCH', 'Sentuhan cepat'); showToast('⚠️ Terlalu cepat!', 'warning', 3000); touchCount = 0; } } else touchCount = 0; touchStart = n; });
...
// ==================== CEK RESET (FIREBASE CACHE) ====================
async function cekResetUjian(u, m, j) { try { const coll = window.Firebase.collection(window.db, 'cache_reset'); const snap = await window.Firebase.getDocs(coll); for (const d of snap.docs) { const row = d.data(); if (row.username === u && (row.mapel === m || row.mapel === '*' || row.mapel === 'SEMUA') && (row.jenis === j || row.jenis === '*' || row.jenis === 'SEMUA') && row.reset === 'YA') return true; } } catch(e) {} return false; }

// ==================== LOGIN (FIREBASE CACHE) ====================
...
// ==================== LOAD UJIAN AKTIF (FIREBASE CACHE) ====================
async function loadUjianAktif(siswa) {
    ...
    try {
        ...
        const tokenMap = {}; tokenSnap.forEach(d => { const t = d.data(); tokenMap[t.token] = t; });
        ...
        for (const docSnap of jadwalSnap.docs) {
            ...
            if (row.waktuMulai && row.waktuSelesai) { mulaiObj = parseWaktu(row.waktuMulai, tgl); selesaiObj = parseWaktu(row.waktuSelesai, tgl); if (mulaiObj && selesaiObj) statusWaktu = now < mulaiObj ? 'belum_mulai' : (now > selesaiObj ? 'selesai' : 'berlangsung'); }
            ...
            daftarUjianAktif.push({ token: row.token, mapel: mapelCek, jenis: jenisCek, tanggal: hariIni, waktuMulai: row.waktuMulai, waktuSelesai: row.waktuSelesai, waktuMulaiObj: mulaiObj, waktuSelesaiObj: selesaiObj, statusWaktu, sudahSelesai, nilaiData, password: row.password || '' });
        }
...
// ==================== PROSES UJIAN DIPILIH (FIREBASE CACHE) ====================
async function prosesUjianDipilih(u) {
    ...
    try {
        ...
        if (!(await cekResetUjian(siswa.username, info.mapel, info.jenis)) && window.db) {
            ...
            if (!sesiSnap.empty) { const sesi = sesiSnap.docs[0].data(); sesi.idSesi = sesiSnap.docs[0].id; pendingUser = siswa; pendingUjian = jadwalLengkap; pendingWaktuSelesai = wS; pendingSesiAktif = sesi; clearInterval(countdownInterval); document.getElementById("dashboardScreen").style.display = "none"; document.getElementById("resumeScreen").style.display = "block"; document.getElementById("resumeMapel").textContent = info.mapel; document.getElementById("resumeJenis").textContent = info.jenis; const sisa = Math.max(Math.floor((wS - now) / 1000), 0); document.getElementById("resumeSisaWaktu").textContent = `${Math.floor(sisa/3600)}j ${Math.floor((sisa%3600)/60)}m`; return; }
        ...
        document.getElementById("confirmNIS").textContent = siswa.nis; document.getElementById("confirmNama").textContent = siswa.nama; document.getElementById("confirmKelas").textContent = siswa.kelas; document.getElementById("confirmMapel").textContent = `${info.mapel} - ${info.jenis}`; document.getElementById("confirmWaktu").textContent = `${jadwal.waktuMulai||'-'} - ${jadwal.waktuSelesai||'-'}`;
...
async function ambilSoal(j, m, js) {
    try {
        ...
        const snap = await window.Firebase.getDocs(q);
        let mentah = [];
        snap.forEach(d => { const row = d.data(); mentah.push({ id: row.id, tipe: row.tipe, pertanyaan: row.pertanyaan, pilihan: [row.pilihanA, row.pilihanB, row.pilihanC, row.pilihanD, row.pilihanE].filter(p => p), kunci: row.kunci, bobot: row.bobot, gambar: row.gambar, grupSoal: row.grupSoal }); });
...
function renderSoal(idx) {
    ...
    if (s.tipe === "PG") { s.pilihan.forEach((o,i) => { const hu = String.fromCharCode(65+i); h += `<label class="option-label"><input type="radio" name="jwb" value="${hu}" ${jaw===hu?"checked":""} ${isFrozen?"disabled":""} onchange="autoSavePG('${s.id}')"> ${hu}. ${o}</label>`; }); h += `<button class="btn-simpan" onclick="simpanPG('${s.id}')" ${isFrozen?"disabled":""}><i class="fas fa-save"></i> Simpan</button>`; }
        ...
        istilahList.forEach((key, idx) => { const namaIstilah = istilahMap[key] || key; const selected = o[key] || ''; h += `<div style="background:white;padding:12px;border-radius:10px;margin-bottom:8px;border:2px solid #FDE68A;"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;"><span style="font-weight:700;color:#1E3A8A;">${idx+1}.</span><span style="font-weight:600;color:#1E293B;font-size:13px;flex:1;">${namaIstilah}</span><select id="jodoh_${s.id}_${key}" onchange="autoSaveJodohDropdown('${s.id}')" style="padding:8px;border-radius:8px;border:2px solid #E2E8F0;font-size:13px;" ${isFrozen?'disabled':''}><option value="">-- Pilih --</option>`; opsi.forEach((opt, i) => { const hu = String.fromCharCode(65 + i); const usedByOther = Object.entries(o).some(([k, v]) => k !== key && v === hu); h += `<option value="${hu}" ${selected === hu ? 'selected' : ''} ${usedByOther ? 'disabled' : ''}>${hu}. ${opt.replace(/^[A-E]\.\s*/, '')}${usedByOther ? ' (dipakai)' : ''}</option>`; }); h += `</select></div></div>`; });
        h += `</div><div style="flex:1;min-width:240px;background:#E8F4FD;padding:14px;border-radius:14px;border:2px solid #BFDBFE;"><div style="font-weight:700;margin-bottom:10px;color:#1E40AF;">📦 DAFTAR JAWABAN</div>`; opsi.forEach((opt, i) => { const hu = String.fromCharCode(65 + i); const isUsed = Object.values(o).includes(hu); h += `<div style="background:${isUsed?'#DCFCE7':'white'};padding:10px 12px;border-radius:8px;margin-bottom:6px;border:1px solid ${isUsed?'#BBF7D0':'#E2E8F0'};font-size:13px;"><strong style="color:#1E3A8A;">${hu}.</strong> ${opt.replace(/^[A-E]\.\s*/, '')}${isUsed ? ' ✅' : ''}</div>`; }); h += `</div></div>`;
...
function autoSaveJodohDropdown(id){const soal=dataSoal.find(q=>q.id===id);if(!soal)return;let k={};try{k=JSON.parse(soal.kunci)}catch(e){return}const o={};let allFilled=true;const totalKey=Object.keys(k).length;Object.keys(k).forEach(key=>{const select=document.getElementById(`jodoh_${id}_${key}`);if(select&&select.value){o[key]=select.value}else{allFilled=false}});if(allFilled){jawabanLokal[id]=JSON.stringify(o);renderNavigator();simpanKeLocalStorage();let benar=0;for(let key in k)if(o[key]===k[key])benar++;simpanJawabanKeFirebase(id,JSON.stringify(o),(benar/totalKey)*soal.bobot);showToast(`✅ Tersimpan otomatis! (${benar}/${totalKey} benar)`,'success',1500)}else{showToast(`📝 ${Object.keys(o).length}/${totalKey} terjawab`,'info',1000)}}
function simpanJodohDropdown(id){const soal=dataSoal.find(q=>q.id===id);if(!soal)return;let k={};try{k=JSON.parse(soal.kunci)}catch(e){showError('Format kunci salah!');return}const o={};const totalKey=Object.keys(k).length;Object.keys(k).forEach(key=>{const select=document.getElementById(`jodoh_${id}_${key}`);if(select&&select.value)o[key]=select.value});const filledKey=Object.keys(o).length;if(filledKey<totalKey){showError(`Baru ${filledKey} dari ${totalKey} yang dijawab!`);return}jawabanLokal[id]=JSON.stringify(o);renderNavigator();simpanKeLocalStorage();let benar=0;for(let key in k)if(o[key]===k[key])benar++;simpanJawabanKeFirebase(id,JSON.stringify(o),(benar/totalKey)*soal.bobot);showSuccess(`✅ Jawaban tersimpan! (${benar}/${totalKey} benar)`)}
...
// ==================== TIMER ====================
function mulaiTimer() { if (!waktuSelesai) return; if (timerInterval) clearInterval(timerInterval); function tick() { const s = Math.max(waktuSelesai - new Date(), 0), d = Math.floor(s/1000); document.getElementById("timerDisplay").innerText = `${Math.floor(d/60)}:${String(d%60).padStart(2,'0')}`; if (d === 0 && !ujianSelesai) { ujianSelesai = true; clearInterval(timerInterval); showModal({ iconType: "warning", title: "Waktu Habis", message: "Ujian berakhir.", buttons: [{ text: "OK", type: "primary", onClick: selesaiUjian }] }); } } tick(); timerInterval = setInterval(tick, 1000); }
function konfirmasiSelesai() { if (isFrozen) return; const b = dataSoal.filter(s => !jawabanLokal[s.id]).length; showModal({ iconType: "warning", title: "Akhiri Ujian?", message: `📝 ${dataSoal.length-b} dijawab\n⚠️ ${b} belum`, showCheckbox: true, checkboxLabel: "Saya yakin", buttons: [{ text: "Lanjutkan", type: "secondary" }, { text: "Ya, Selesai", type: "warning", onClick: c => { if (!c) showError("Centang!"); else selesaiUjian(); } }] }); }
...
async function selesaiUjian() {
    ...
    dataSoal.forEach(s => { const b = s.bobot || 1; totalBobot += b; const j = jawabanLokal[s.id]; if (!j) return; if (s.tipe === "PG" && j === s.kunci) { totalSkor += b; jumlahBenar++; } else if (s.tipe === "PGK") { try { if (JSON.stringify(JSON.parse(j).sort()) === JSON.stringify(JSON.parse(s.kunci).sort())) { totalSkor += b; jumlahBenar++; } } catch(e){} } else if (s.tipe === "B/S") { try { if (j.startsWith('[')) { const ja = JSON.parse(j), ka = JSON.parse(s.kunci); let x = 0; for (let i = 0; i < ka.length; i++) if (ja[i] === ka[i]) x++; totalSkor += (x/ka.length)*b; if (x === ka.length) jumlahBenar++; } else { if (j === s.kunci) { totalSkor += b; jumlahBenar++; } } } catch(e) { if (j === s.kunci) { totalSkor += b; jumlahBenar++; } } } else if (s.tipe === "Jodoh") { try { const jo = JSON.parse(j), ko = JSON.parse(s.kunci); let benar = 0; const tt = Object.keys(ko).length; for (let key in ko) if (jo[key] === ko[key]) benar++; totalSkor += (benar/tt)*b; if (benar === tt) jumlahBenar++; } catch(e){} } else if (s.tipe === "Isian") { const jn = String(j).toLowerCase().replace(/\s+/g,' ').trim(); const kn = String(s.kunci).toLowerCase().replace(/\s+/g,' ').trim(); if (kn === '*' || kn === 'semua' || kn === 'all') { totalSkor += b; jumlahBenar++; } else if (kn.includes('|')) { if (kn.split('|').map(k=>k.trim()).includes(jn)) { totalSkor += b; jumlahBenar++; } } else if (jn === kn) { totalSkor += b; jumlahBenar++; } } });
    ...
    if (window.db) { try { const nilaiData = { idSesi, username: currentUser?.username || pendingUser?.username, nis: currentUser?.nis || pendingUser?.nis, nama: currentUser?.nama || pendingUser?.nama, kelas: currentUser?.kelas || pendingUser?.kelas, jenjang: currentUser?.jenjang || pendingUser?.jenjang, mapel: currentUjian?.mapel || pendingUjian?.mapel, jenisUjian: currentUjian?.jenis || pendingUjian?.jenis, totalSkor, jumlahBenar, jumlahSoal: dataSoal.length, totalBobot, persentase: persen+'%', timestamp: new Date().toISOString() }; const q = window.Firebase.query(window.Firebase.collection(window.db,'nilai_akhir'), window.Firebase.where('idSesi','==',idSesi)); const snap = await window.Firebase.getDocs(q); if (!snap.empty) await window.Firebase.updateDoc(snap.docs[0].ref, nilaiData); else await window.Firebase.addDoc(window.Firebase.collection(window.db,'nilai_akhir'), nilaiData); } catch(e){} try { const q = window.Firebase.query(window.Firebase.collection(window.db,'sesi_ujian'), window.Firebase.where('idSesi','==',idSesi)); const snap = await window.Firebase.getDocs(q); snap.forEach(d => window.Firebase.updateDoc(d.ref, { status: 'Selesai', waktuSelesai: new Date().toISOString() })); } catch(e){} }
