...
function renderSoal(idx) {
    ...
    const istilahList = Object.keys(k), opsi = s.pilihan.filter(p => p && p.trim());
    ...
    h += `<div style="background:#FEF3C7;padding:16px;border-radius:16px;margin-bottom:12px;"><div style="font-weight:600;margin-bottom:12px;color:#92400E;">📋 DAFTAR ISTILAH</div>`;
    istilahList.forEach((key, idx) => { const selected = o[key] || ''; h += `<div style="background:white;padding:14px;border-radius:12px;margin-bottom:10px;border:2px solid #FDE68A;"><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;"><span style="font-weight:700;color:#1E3A8A;min-width:30px;">${idx+1}.</span><span style="font-weight:600;color:#1E293B;flex:1;min-width:120px;">${key}</span><span style="color:#64748B;font-size:20px;">→</span><select id="jodoh_${s.id}_${key}" onchange="autoSaveJodohDropdown('${s.id}')" style="flex:1;min-width:150px;padding:10px 12px;border-radius:10px;border:2px solid #E2E8F0;font-size:14px;background:white;" ${isFrozen?'disabled':''}><option value="">-- Pilih Jawaban --</option>`; opsi.forEach((opt, i) => { const hu = String.fromCharCode(65 + i); const usedByOther = Object.entries(o).some(([k, v]) => k !== key && v === hu); h += `<option value="${hu}" ${selected === hu ? 'selected' : ''} ${usedByOther ? 'disabled style="color:#CBD5E1;"' : ''}>${hu}. ${opt.replace(/^[A-E]\.\s*/, '')}${usedByOther ? ' (sudah dipakai)' : ''}</option>`; }); h += `</select></div></div>`; });
    h += `</div><div style="background:#DBEAFE;padding:16px;border-radius:16px;margin-bottom:12px;"><div style="font-weight:600;margin-bottom:12px;color:#1E40AF;">📦 DAFTAR JAWABAN</div>`; opsi.forEach((opt, i) => { const hu = String.fromCharCode(65 + i); const isUsed = Object.values(o).includes(hu); h += `<div style="background:${isUsed?'#DCFCE7':'white'};padding:10px 14px;border-radius:10px;margin-bottom:6px;border:1px solid ${isUsed?'#BBF7D0':'#E2E8F0'};"><strong style="color:#1E3A8A;">${hu}.</strong> ${opt.replace(/^[A-E]\.\s*/, '')}${isUsed ? '<span style="color:#16A34A;margin-left:8px;font-size:12px;">✅ Dipilih</span>' : ''}</div>`; }); h += `</div>`;
    h += `<button class="btn-simpan" onclick="simpanJodohDropdown('${s.id}')" ${isFrozen?'disabled':''}><i class="fas fa-save"></i> Simpan Jawaban</button>`;
}
