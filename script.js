...
let pendingUser = null, pendingUjian = null, pendingWaktuSelesai = null, pendingSesiAktif = null;
...
document.addEventListener("fullscreenchange", () => { if (!document.fullscreenElement) { if (currentUser && !ujianSelesai) { isFullscreen = false; catatPelanggaran("FULLSCREEN_EXIT", "Keluar fullscreen"); freezeScreen(60); showFullscreenPrompt(); } } else isFullscreen = true; });

// ==================== ANTI-CURANG ====================
window.addEventListener('pagehide', () => { isLocked = true; });
window.addEventListener('pageshow', () => { if (isLocked) { isLocked = false; } });
document.addEventListener("visibilitychange", () => { if (document.hidden && currentUser && !ujianSelesai && !isFrozen) { if (!isLocked) { catatPelanggaran("TAB_SWITCH", "Pindah tab"); freezeScreen(60); document.getElementById("alertOverlay").style.display = "block"; document.getElementById("alertSound").play(); setTimeout(() => document.getElementById("alertOverlay").style.display = "none", 3000); } } });
...
function updateTombolSelesai() {
    ...
    const sekarang = new Date(), menitBerlalu = Math.floor((sekarang - waktuMulaiServer) / 60000), sisaMenit = Math.max(minimalMenit - menitBerlalu, 0);
    if (sisaMenit > 0 && !ujianSelesai) { tombolSelesaiAktif = false; btn.disabled = true; btn.innerHTML = `<i class="fas fa-lock"></i> (${sisaMenit}m)`; }
    else { tombolSelesaiAktif = true; btn.disabled = false; btn.innerHTML = `<i class="fas fa-check-circle"></i>`; }
}
...
async function handleLogin() {
    ...
    try {
        ...
        const now = new Date(), [hS, mS] = String(jadwal.selesai).split(':').map(n => parseInt(n)), wS = new Date(); wS.setHours(hS, mS, 0, 0);
        if (now >= wS) { showError("Waktu ujian sudah berakhir!"); return; }

        // Cek sesi aktif
        const sesiRes = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${CONFIG.SPREADSHEET_ID}/values/SESI_UJIAN!A:K?key=${CONFIG.API_KEY}`), sesiData = await sesiRes.json(), sesiRows = sesiData.values || [];
        let sesiAktif = null;
        for (let i = 1; i < sesiRows.length; i++) {
            if (sesiRows[i][1] === siswa.username && sesiRows[i][7] === 'Aktif' && sesiRows[i][3] === tokenInfo.mapel && sesiRows[i][4] === tokenInfo.jenis) {
                sesiAktif = { idSesi: sesiRows[i][0], waktuMulai: new Date(sesiRows[i][5]), jenjang: sesiRows[i][2], mapel: sesiRows[i][3], jenis: sesiRows[i][4], pelanggaran: sesiRows[i][9] || 0 }; break;
            }
        }
        if (sesiAktif) {
            pendingUser = siswa; pendingUjian = { ...tokenInfo, ...jadwal }; pendingWaktuSelesai = wS; pendingSesiAktif = sesiAktif;
            const sisaDetik = Math.max(Math.floor((wS - now) / 1000), 0), sisaMenit = Math.floor(sisaDetik / 60);
            const jawabanRes = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${CONFIG.SPREADSHEET_ID}/values/JAWABAN_SISWA!A:I?key=${CONFIG.API_KEY}`), jawabanData = await jawabanRes.json(), jawabanRows = jawabanData.values || [];
            let terjawab = 0; for (let i = 1; i < jawabanRows.length; i++) if (jawabanRows[i][0] === sesiAktif.idSesi) terjawab++;
            document.getElementById("loginScreen").style.display = "none"; document.getElementById("resumeScreen").style.display = "block";
            document.getElementById("resumeMapel").textContent = tokenInfo.mapel; document.getElementById("resumeJenis").textContent = tokenInfo.jenis;
            document.getElementById("resumeSisaWaktu").textContent = `${Math.floor(sisaMenit/60)}j ${sisaMenit%60}m`; document.getElementById("resumeProgress").textContent = `${terjawab} soal`;
            return;
        }
...
function cancelResume() {
    ...
    if (pendingSesiAktif) { fetch(CONFIG.PROXY_URL, { method: "POST", body: JSON.stringify({ action: "updateStatusSesi", idSesi: pendingSesiAktif.idSesi, status: "Dibatalkan" }) }); }
    document.getElementById("resumeScreen").style.display = "none"; document.getElementById("confirmScreen").style.display = "block";
    document.getElementById("confirmNIS").textContent = pendingUser.nis || '-'; document.getElementById("confirmNama").textContent = pendingUser.nama || '-';
    document.getElementById("confirmKelas").textContent = pendingUser.kelas || '-'; document.getElementById("confirmMapel").textContent = `${pendingUjian.mapel} - ${pendingUjian.jenis}`;
...
async function continueExam() {
    ...
    idSesi = pendingSesiAktif.idSesi; waktuMulaiServer = new Date(pendingSesiAktif.waktuMulai); totalPenalti = pendingSesiAktif.pelanggaran || 0;
    ...
    await ambilSoal(currentUser.jenjang, currentUjian.mapel, currentUjian.jenis); await loadJawabanDariServer();
    mulaiTimer(); renderNavigator(); showFullscreenPrompt(); updateTombolSelesai(); setInterval(updateTombolSelesai, 1000);
    showSuccess(`Selamat datang kembali, ${currentUser.nama || 'Siswa'}! ${Object.keys(jawabanLokal).length} jawaban dimuat.`);
}
...
async function loadJawabanDariServer() {
    ...
    try {
        const url = `https://sheets.googleapis.com/v4/spreadsheets/${CONFIG.SPREADSHEET_ID}/values/JAWABAN_SISWA!A:I?key=${CONFIG.API_KEY}`, res = await fetch(url), data = await res.json(), rows = data.values || [];
        let loaded = 0;
        for (let i = 1; i < rows.length; i++) if (rows[i][0] === idSesi) { jawabanLokal[rows[i][5]] = rows[i][6]; loaded++; }
...
function acakSoalDenganGrup(soalList) {
    ...
    soalList.forEach(soal => { if (soal.grupSoal && soal.grupSoal.trim() !== '') { const g = soal.grupSoal.trim(); if (!soalDenganGrup[g]) soalDenganGrup[g] = []; soalDenganGrup[g].push(soal); } else soalTanpaGrup.push(soal); });
    const grupBlocks = []; for (let g in soalDenganGrup) grupBlocks.push(soalDenganGrup[g]);
    const semuaBlocks = [...grupBlocks, ...soalTanpaGrup.map(s => [s])], blocksTeracak = shuffleArray(semuaBlocks), hasil = [];
    blocksTeracak.forEach(block => { block.forEach(soal => hasil.push(soal)); }); return hasil;
}
async function ambilSoal(j, m, js) {
    try {
        ...
        let soalMentah = [];
        for (let i = 1; i < rows.length; i++) if (String(rows[i][1]).trim() === String(j).trim() && String(rows[i][2]).trim() === String(m).trim() && String(rows[i][3]).trim() === String(js).trim() && rows[i][13] === "Aktif") {
        ...
        dataSoal = acakSoalDenganGrup(soalMentah); renderSoal(0);
...
function renderNavigator() {
    let h = "";
    for (let i = 0; i < dataSoal.length; i++) { const s = dataSoal[i], ans = jawabanLokal[s.id] !== undefined, rag = raguLokal[s.id], cur = i === indexSoal; let c = "unanswered"; if (ans) c = "answered"; if (rag && !cur) c = "ragu"; if (cur) c = "current"; h += `<button class="nav-btn-num ${c}" onclick="goToSoal(${i})">${i + 1}</button>`; }
...
function renderSoal(idx) {
    ...
    else if (s.tipe === "B/S") { const pernyataan = s.pilihan.filter(p => p && p.trim() !== ''); if (pernyataan.length === 0) { h += `<div style="background:#f8fafc; padding:16px; border-radius:12px; margin-bottom:16px;"><p>${s.pertanyaan}</p><div style="display:flex; gap:24px;"><label><input type="radio" name="bs_single" value="B" ${jaw === 'B' ? 'checked' : ''} onchange="autoSaveBSSingle('${s.id}')"> ✅ BENAR</label><label><input type="radio" name="bs_single" value="S" ${jaw === 'S' ? 'checked' : ''} onchange="autoSaveBSSingle('${s.id}')"> ❌ SALAH</label></div></div>`; h += `<button class="btn-simpan" onclick="simpanBSSingle('${s.id}')"><i class="fas fa-save"></i> Simpan</button>`; } else { let jawabanArr = []; try { if (jaw) jawabanArr = JSON.parse(jaw); } catch (e) { if (jaw === "B" || jaw === "S") jawabanArr = [jaw]; } pernyataan.forEach((teks, i) => { const jwb = jawabanArr[i] || ''; h += `<div style="background:#f8fafc; padding:14px; border-radius:12px; margin-bottom:12px;"><p>${i + 1}. ${teks}</p><div style="display:flex; gap:20px;"><label><input type="radio" name="bs_${i}" value="B" ${jwb === 'B' ? 'checked' : ''} onchange="autoSaveBS('${s.id}', ${pernyataan.length})"> ✅ Benar</label><label><input type="radio" name="bs_${i}" value="S" ${jwb === 'S' ? 'checked' : ''} onchange="autoSaveBS('${s.id}', ${pernyataan.length})"> ❌ Salah</label></div></div>`; }); h += `<button class="btn-simpan" onclick="simpanBS('${s.id}', ${pernyataan.length})"><i class="fas fa-save"></i> Simpan</button>`; } }
    else if (s.tipe === "Jodoh") { let kunciObj = {}, jawabanObj = {}; try { kunciObj = JSON.parse(s.kunci); jawabanObj = JSON.parse(jaw || "{}"); } catch(e) {} window.currentMatchingSoal = s; window.currentMatchingJawaban = jawabanObj; const opsiJawaban = s.pilihan.filter(p => p && p.trim()), hurufMapping = {}; opsiJawaban.forEach((opt, i) => { hurufMapping[String.fromCharCode(65 + i)] = opt; }); window.hurufMapping = hurufMapping; h += `<div style="margin-bottom:12px; padding:10px; background:#e8f0fe; border-radius:12px;"><p style="font-weight:600; color:#1E3A8A;"><i class="fas fa-info-circle"></i> Tarik jawaban dari KANAN ke istilah di KIRI.</p></div><div class="matching-jodoh-container" style="display:flex; gap:16px;">`; h += `<div style="flex:1; background:#FEF3C7; padding:12px; border-radius:16px;"><div style="font-weight:600; margin-bottom:12px; text-align:center;">🎯 ISTILAH</div>`; for (let key in kunciObj) { const isFilled = jawabanObj[key] !== undefined, huruf = jawabanObj[key] || '', teks = hurufMapping[huruf] || ''; h += `<div class="matching-target ${isFilled ? 'filled' : 'empty'}" data-key="${key}" id="target_${key.replace(/[^a-zA-Z0-9]/g,'')}" style="background:white; padding:12px; border-radius:12px; margin-bottom:8px; border:2px dashed #D97706;"><div style="text-align:center;">${isFilled ? `<strong>${key}</strong><br><span style="color:#16a34a;">✅ ${huruf}. ${teks}</span>` : `<strong>${key}</strong><br><span style="color:#94a3b8;">⬅️ Tarik jawaban</span>`}</div></div>`; } h += `</div>`; h += `<div style="flex:1; background:#DBEAFE; padding:12px; border-radius:16px;"><div style="font-weight:600; margin-bottom:12px; text-align:center;">📦 JAWABAN (Tarik ke kiri)</div>`; opsiJawaban.forEach((opt, i) => { const huruf = String.fromCharCode(65 + i), isUsed = Object.values(jawabanObj).includes(huruf); if (!isUsed) h += `<div class="matching-item-right" draggable="true" data-huruf="${huruf}" id="drag_${huruf}" style="background:white; padding:12px; border-radius:12px; margin-bottom:8px; border:2px solid #1E3A8A; cursor:grab;"><strong style="color:#1E3A8A;">${huruf}.</strong> ${opt}</div>`; else h += `<div class="matching-item-right paired" draggable="false" id="drag_${huruf}" style="background:#DCFCE7; padding:12px; border-radius:12px; margin-bottom:8px; border:2px solid #22C55E;"><strong style="color:#1E3A8A;">${huruf}.</strong> ${opt} <span style="color:#16a34a;">✓</span></div>`; }); h += `</div></div><div style="display:flex; gap:12px; margin-top:16px;"><button class="btn-reset-matching" onclick="resetMatching()" style="flex:1;"><i class="fas fa-undo"></i> Reset</button><button class="btn-simpan" onclick="simpanJodohDrag('${s.id}')" style="flex:1;"><i class="fas fa-save"></i> Simpan</button></div>`; setTimeout(() => initDragDropJodoh(), 50); }
...
function debounceAutoSaveIsian(id) { clearTimeout(debounceTimer); debounceTimer = setTimeout(() => { const i = document.getElementById('isian'); if (!i || !i.value.trim()) return; const jwb = i.value.trim(); jawabanLokal[id] = jwb; renderNavigator(); const soal = dataSoal.find(q => q.id === id); const jawabanNormal = jwb.toLowerCase().replace(/\s+/g, ' ').trim(), kunciNormal = soal.kunci.toLowerCase().replace(/\s+/g, ' ').trim(); let skor = 0; if (kunciNormal.includes('|')) { const kunciArr = kunciNormal.split('|').map(k => k.trim()); if (kunciArr.includes(jawabanNormal)) skor = soal.bobot; } else if (jawabanNormal === kunciNormal) skor = soal.bobot; simpanKeServer(id, jwb, skor); showToast('Tersimpan', 'success', 800); }, 1000); }
...
function updateMatchingUIJodoh() { const soal = window.currentMatchingSoal, jawabanObj = window.currentMatchingJawaban, hurufMapping = window.hurufMapping || {}; let kunciObj = {}; try { kunciObj = JSON.parse(soal.kunci); } catch(e) {} for (let key in kunciObj) { const target = document.getElementById(`target_${key.replace(/[^a-zA-Z0-9]/g,'')}`); if (target) { const isFilled = jawabanObj[key] !== undefined, huruf = jawabanObj[key] || '', teks = hurufMapping[huruf] || ''; target.className = `matching-target ${isFilled ? 'filled' : 'empty'}`; target.style.background = isFilled ? '#DCFCE7' : 'white'; target.style.border = isFilled ? '2px solid #22C55E' : '2px dashed #D97706'; target.innerHTML = `<div style="text-align:center;">${isFilled ? `<strong>${key}</strong><br><span style="color:#16a34a;">✅ ${huruf}. ${teks}</span>` : `<strong>${key}</strong><br><span style="color:#94a3b8;">⬅️ Tarik jawaban</span>`}</div>`; } } const usedHuruf = Object.values(jawabanObj), opsiJawaban = soal.pilihan.filter(p => p && p.trim()); opsiJawaban.forEach((opt, i) => { const huruf = String.fromCharCode(65 + i), dragItem = document.getElementById(`drag_${huruf}`); if (dragItem) { const isUsed = usedHuruf.includes(huruf); dragItem.className = `matching-item-right${isUsed ? ' paired' : ''}`; dragItem.setAttribute('draggable', !isUsed); dragItem.style.background = isUsed ? '#DCFCE7' : 'white'; dragItem.style.border = isUsed ? '2px solid #22C55E' : '2px solid #1E3A8A'; dragItem.innerHTML = `<strong style="color:#1E3A8A;">${huruf}.</strong> ${opt}${isUsed ? '<span style="color:#16a34a; margin-left:8px;">✓</span>' : ''}`; } }); initDragDropJodoh(); }
...
async function selesaiUjian() {
    ...
    dataSoal.forEach(s => { const j = jawabanLokal[s.id]; const bo = s.bobot || 1; tot += bo; if (!j) return;
        ...
        else if (s.tipe === "Isian") { const jawabanNormal = j.toLowerCase().replace(/\s+/g, ' ').trim(), kunciNormal = s.kunci.toLowerCase().replace(/\s+/g, ' ').trim(); if (kunciNormal.includes('|')) { const kunciArr = kunciNormal.split('|').map(k => k.trim()); if (kunciArr.includes(jawabanNormal)) { t += bo; b++; } } else if (jawabanNormal === kunciNormal) { t += bo; b++; } }
    });
