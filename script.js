...
let minimalMenit = 45, ujianSelesai = false, idSesi = null;
let pelanggaranCount = 0, totalPenalti = 0;
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden && currentUser && !ujianSelesai) {
        catatPelanggaran('TAB_SWITCH', 'Membuka tab/aplikasi lain');
...
document.addEventListener('contextmenu', e => { e.preventDefault(); if (currentUser && !ujianSelesai) catatPelanggaran('RIGHT_CLICK', 'Klik kanan'); });

document.addEventListener('keydown', e => {
    if (!currentUser || ujianSelesai) return;
    if (e.key === 'F11' || e.key === 'Escape') e.preventDefault();
    if (e.ctrlKey && (e.key === 'w' || e.key === 't' || e.key === 'n')) { e.preventDefault(); catatPelanggaran('KEYBOARD_SHORTCUT', `Ctrl+${e.key.toUpperCase()}`); }
});
...
function goToSoal(i) { renderSoal(i); renderNavigator(); if (window.innerWidth <= 600) document.getElementById('navPanel').classList.remove('show'); }
...
function renderSoal(idx) {
    ...
    if (s.tipe === 'PG') {
        ...
        h += `<button class="btn-simpan" onclick="simpanPG('${s.id}')">Simpan Jawaban</button>`;
    } else if (s.tipe === 'PGK') {
        ...
        h += `<button class="btn-simpan" onclick="simpanPGK('${s.id}')">Simpan Jawaban</button>`;
    } else if (s.tipe === 'B/S') {
        ...
        h += `<button class="btn-simpan" onclick="simpanBS('${s.id}',${pernyataan.length})">Simpan Jawaban</button>`;
        ...
        for (let k in kunci) { h += `<div>${k}</div><div><input type="text" id="jodoh_${k.replace(/\s/g,'')}" value="${jawObj[k]||''}" placeholder="A/B/C/D" class="jodoh-input"></div>`; }
        h += `</div><button class="btn-simpan" onclick="simpanJodoh('${s.id}','${s.kunci.replace(/"/g,'&quot;')}')">Simpan Jawaban</button>`;
    } else if (s.tipe === 'Isian') {
        h += `<input type="text" id="isian" value="${jaw||''}" placeholder="Ketik jawaban Anda..." style="width:100%;padding:14px;border-radius:16px;border:1px solid #e2e8f0;">`;
        h += `<button class="btn-simpan" onclick="simpanIsian('${s.id}')">Simpan Jawaban</button>`;
    }
...
function konfirmasiSelesai() {
    ...
    showModal({
        ...
        buttons: [
            ...
            { text: 'Ya, Selesai', type: 'warning', onClick: (checked) => { if (!checked) { showError('Centang konfirmasi!'); return false; } selesaiUjian(); } }
        ]
    });
}

async function selesaiUjian() {
    clearInterval(timerInterval); ujianSelesai = true;
    let total=0, benar=0;
    dataSoal.forEach(s=>{
        const j=jawabanLokal[s.id]; if(!j) return;
    ...
    const persentase = ((total / dataSoal.length) * 100).toFixed(1);
    showModal({
        iconType: 'success', title: '🎉 Ujian Selesai!', message: '',
        buttons: [{ text: 'Tutup', type: 'success', onClick: () => location.reload() }]
    });
    setTimeout(() => {
        document.querySelector('.modal-message').innerHTML = `
            <div class="score-summary">
                <div class="score-number">${total.toFixed(2)}</div>
                <div class="score-label">Total Skor</div>
                <div class="score-details">
                    <div class="score-detail-item"><div class="score-detail-value">${benar}/${dataSoal.length}</div><div class="score-detail-label">Soal Benar</div></div>
                    <div class="score-detail-item"><div class="score-detail-value">${persentase}%</div><div class="score-detail-label">Persentase</div></div>
                </div>
                ${totalPenalti > 0 ? `<p style="margin-top:16px; color:#fca5a5;">⏱️ Total penalti: ${totalPenalti} menit</p>` : ''}
            </div>
            <p style="margin-top:8px;">Jawaban Anda telah tersimpan.</p>
        `;
    }, 10);
}
