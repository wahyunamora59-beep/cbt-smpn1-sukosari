...
function togglePassword() {
    ...
    if (i) {
        ...
        if (icon) icon.className = i.type === "password" ? "fas fa-eye toggle-password-modern" : "fas fa-eye-slash toggle-password-modern";
    }
}
...
function parseWaktu(waktuStr, tanggalRef) {
    ...
    const parts = String(waktuStr).trim().split(':');
    if (parts.length >= 2) {
        const jam = parseInt(parts[0]), menit = parseInt(parts[1]);
        const t = tanggalRef ? new Date(tanggalRef) : new Date();
...
document.addEventListener("keydown", e => { if (!currentUser || ujianSelesai || isFrozen) return; if (e.key === "F11" || e.key === "Escape" || e.key === "PrintScreen") { e.preventDefault(); if (e.key === "PrintScreen") { catatPelanggaran('PRINT_SCREEN', 'Mencoba screenshot'); freezeScreen(); showToast('🚫 Screenshot tidak diizinkan!', 'error'); } return false; } if (e.ctrlKey && ['w','t','n','p','c','v','a','x'].includes(e.key.toLowerCase())) { e.preventDefault(); catatPelanggaran("KEYBOARD", `Ctrl+${e.key.toUpperCase()}`); showToast(`🚫 Shortcut Ctrl+${e.key.toUpperCase()} tidak diizinkan!`, 'warning'); freezeScreen(); return false; } });
window.addEventListener("beforeunload", e => { if (currentUser && !ujianSelesai) { e.preventDefault(); e.returnValue = ""; } });
function catatPelanggaran(j, d) { if (!currentUser || ujianSelesai) return; pelanggaranCount++; totalPenalti++; console.warn(`⚠️ Pelanggaran: ${j} - ${d}`); }

// ==================== ANTI-FLOATING APPS ====================
let blurCount = 0, lastBlurTime = 0, lastWindowSize = { width: window.innerWidth, height: window.innerHeight }, resizeCheckTimer = null, touchStartTime = 0, touchCount = 0, devtoolsOpen = false, lastVisibilityChange = 0;
const MAX_BLUR_COUNT = 3, BLUR_COOLDOWN = 5000, threshold = 160, VISIBILITY_COOLDOWN = 3000;

window.addEventListener('blur', () => { if (!currentUser || ujianSelesai || isFrozen) return; const now = Date.now(); if (now - lastBlurTime < BLUR_COOLDOWN) return; lastBlurTime = now; blurCount++; catatPelanggaran('WINDOW_BLUR', `Keluar jendela (${blurCount}/${MAX_BLUR_COUNT})`); freezeScreen(30); document.getElementById('alertOverlay').style.display = 'block'; document.getElementById('alertSound').play(); showToast(`⚠️ Jangan buka aplikasi lain!`, 'error', 5000); if (blurCount >= MAX_BLUR_COUNT) showModal({ iconType: 'error', title: '⛔ UJIAN DIBATALKAN', message: `Anda ${blurCount}x membuka aplikasi lain.`, buttons: [{ text: 'OK', type: 'primary', onClick: () => selesaiUjian() }] }); setTimeout(() => document.getElementById('alertOverlay').style.display = 'none', 3000); });
window.addEventListener('resize', () => { if (!currentUser || ujianSelesai || isFrozen) return; clearTimeout(resizeCheckTimer); resizeCheckTimer = setTimeout(() => { const nW = window.innerWidth, nH = window.innerHeight, sW = screen.width, sH = screen.height; if ((Math.abs(nW - lastWindowSize.width) / lastWindowSize.width > 0.4 || Math.abs(nH - lastWindowSize.height) / lastWindowSize.height > 0.4) && isFullscreen) { catatPelanggaran('RESIZE_DRASTIC', `Ukuran berubah drastis`); showToast('⚠️ Perubahan ukuran mencurigakan!', 'warning', 4000); } lastWindowSize = { width: nW, height: nH }; }, 500); });
document.addEventListener('mouseleave', e => { if (!currentUser || ujianSelesai || isFrozen) return; if (e.clientY <= 0 || e.clientX <= 0 || e.clientX >= window.innerWidth || e.clientY >= window.innerHeight) { const now = Date.now(); if (now - lastBlurTime < BLUR_COOLDOWN) return; lastBlurTime = now; catatPelanggaran('MOUSE_LEAVE', 'Kursor keluar'); showToast('👆 Kursor keluar area!', 'warning', 3000); } });
document.addEventListener('touchstart', e => { if (!currentUser || ujianSelesai || isFrozen) return; if (e.touches.length > 3) { catatPelanggaran('MULTI_TOUCH', `${e.touches.length} jari`); showToast(`⚠️ Multi-touch ${e.touches.length} jari!`, 'warning', 3000); } const now = Date.now(); if (now - touchStartTime < 100) { touchCount++; if (touchCount > 5) { catatPelanggaran('RAPID_TOUCH', 'Sentuhan cepat'); showToast('⚠️ Sentuhan terlalu cepat!', 'warning', 3000); touchCount = 0; } } else touchCount = 0; touchStartTime = now; });
function detectDevTools() { if (!currentUser || ujianSelesai) return; if ((window.outerWidth - window.innerWidth > threshold || window.outerHeight - window.innerHeight > threshold) && !devtoolsOpen) { devtoolsOpen = true; catatPelanggaran('DEVTOOLS_OPEN', 'DevTools terbuka'); freezeScreen(45); showToast('🚫 Developer tools terdeteksi!', 'error', 5000); } else devtoolsOpen = false; }
setInterval(detectDevTools, 1000);
document.addEventListener('visibilitychange', () => { if (document.hidden && currentUser && !ujianSelesai && !isFrozen) { const now = Date.now(); if (now - lastVisibilityChange < VISIBILITY_COOLDOWN) return; lastVisibilityChange = now; if (!isLocked) { catatPelanggaran('TAB_SWITCH', 'Pindah tab'); freezeScreen(30); document.getElementById('alertOverlay').style.display = 'block'; document.getElementById('alertSound').play(); showToast('🚫 Jangan pindah tab!', 'error', 4000); setTimeout(() => document.getElementById('alertOverlay').style.display = 'none', 3000); } } });

// ==================== UPDATE TOMBOL SELESAI ====================
function updateTombolSelesai() { const b = document.querySelector(".btn-selesai-modern"); if (!b || !waktuMulaiServer) return; const s = new Date(), m = Math.floor((s - waktuMulaiServer) / 60000), r = Math.max(minimalMenit - m, 0); if (r > 0 && !ujianSelesai) { tombolSelesaiAktif = false; b.disabled = true; b.innerHTML = `<i class="fas fa-lock"></i> (${r}m)`; } else { tombolSelesaiAktif = true; b.disabled = false; b.innerHTML = `<i class="fas fa-check-circle"></i> SELESAI`; } }

// ==================== PARSE ISTILAH ====================
function parseIstilahDariPertanyaan(teks) {
    const istilahMap = {}; let teksBagianA = teks;
    ['BAGIAN B','Bagian B','Pilihan:','Pilihan Jawaban:','\nA.','\nA)','\nA ','A. if','A. '].forEach(m => { const i = teks.indexOf(m); if (i !== -1) teksBagianA = teks.substring(0, i); });
    const lines = teksBagianA.split('\n'); let cur = null;
    lines.forEach(l => { l = l.trim(); if (!l) return; const m = l.match(/^(\d+)\.\s+(.+)$/); if (m) { cur = m[1]; istilahMap[cur] = m[2]; } else if (cur) istilahMap[cur] += ' ' + l; });
    if (!Object.keys(istilahMap).length) { const r = /(\d+)\.\s*([^\n]+)/g; let m; while ((m = r.exec(teksBagianA)) !== null) istilahMap[m[1]] = m[2].trim(); }
    return istilahMap;
}
...
async function cekResetUjian(username, mapel, jenis) {
    try {
        const r = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${CONFIG.SPREADSHEET_ID}/values/RESET_UJIAN!A:E?key=${CONFIG.API_KEY}`), d = await r.json(), rows = d.values || [];
        for (let i = 1; i < rows.length; i++) {
            const row = rows[i];
            if ((row[0] === username) && (row[1] === mapel || row[1] === '*' || row[1] === 'SEMUA') && (row[2] === jenis || row[2] === '*' || row[2] === 'SEMUA') && row[3] === 'YA') return true;
        }
    } catch (e) {}
    return false;
}
...
async function handleLogin() {
    ...
    try {
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            if (String(row[2] || '').trim() === u) {
                if (String(row[3] || '').trim() !== p) { showError("Password salah!"); return; }
                siswa = { nis: row[0]||'', nama: row[1]||'', username: row[2], kelas: row[4]||'', jenjang: String(row[5]||'').trim() };
                break;
            }
        }
...
async function loadUjianAktif(siswa) {
    ...
    try {
        ...
        for (let i = 1; i < jRows.length; i++) {
            ...
            if (row[5] && row[6]) {
                mulaiObj = parseWaktu(row[5], tgl); selesaiObj = parseWaktu(row[6], tgl);
                if (mulaiObj && selesaiObj) statusWaktu = sekarang < mulaiObj ? 'belum_mulai' : (sekarang > selesaiObj ? 'selesai' : 'berlangsung');
            }
...
function renderUjianList(list) {
    ...
    list.forEach((u, i) => {
        ...
        if (u.sudahSelesai) { status = `✅ Selesai | ${u.nilaiData?.totalSkor?.toFixed(0)||'0'}`; cls = 'status-selesai'; waktu = `<span><i class="fas fa-check-circle"></i> Sudah dikerjakan</span>`; }
    ...
    countdownInterval = setInterval(() => {
        ...
        daftarUjianAktif.forEach((u, i) => {
            ...
            if (now < u.waktuMulaiObj) {
                ...
                const c = span?.querySelector('.countdown'); if (c) c.textContent = `${String(Math.floor(d/3600)).padStart(2,'0')}:${String(Math.floor((d%3600)/60)).padStart(2,'0')}:${String(d%60).padStart(2,'0')}`;
            } else if (now > u.waktuSelesaiObj) {
                span.className = 'ujian-status status-selesai'; span.textContent = '❌ Berakhir'; if (btn) btn.disabled = true; u.statusWaktu = 'selesai';
            } else {
...
async function prosesUjianDipilih(u) {
    ...
    try {
        ...
        if (!info) { showError("Token tidak valid!"); return; }
        ...
        if (!(await cekResetUjian(siswa.username, info.mapel, info.jenis)) && window.db) {
            ...
            if (!sesiSnap.empty) {
                ...
                pendingUser = siswa; pendingUjian = {...info, ...jadwal}; pendingWaktuSelesai = wS; pendingSesiAktif = sesi;
                clearInterval(countdownInterval); document.getElementById("dashboardScreen").style.display = "none"; document.getElementById("resumeScreen").style.display = "block";
                document.getElementById("resumeMapel").textContent = info.mapel; document.getElementById("resumeJenis").textContent = info.jenis;
                const sisa = Math.max(Math.floor((wS - now) / 1000), 0); document.getElementById("resumeSisaWaktu").textContent = `${Math.floor(sisa/3600)}j ${Math.floor((sisa%3600)/60)}m`;
                return;
            }
            ...
            if (!nilaiSnap.empty) {
                ...
                showModal({ iconType: 'warning', title: '⏰ Sudah Selesai', message: `Nilai: ${n.totalSkor?.toFixed(0)||'0'}\nBenar: ${n.jumlahBenar}/${n.jumlahSoal}`, buttons: [{ text: 'OK', type: 'primary' }] });
                return;
            }
        }
        clearInterval(countdownInterval); document.getElementById("dashboardScreen").style.display = "none"; document.getElementById("confirmScreen").style.display = "block";
        pendingUser = siswa; pendingUjian = {...info, ...jadwal}; pendingWaktuSelesai = wS;
        document.getElementById("confirmNIS").textContent = siswa.nis; document.getElementById("confirmNama").textContent = siswa.nama; document.getElementById("confirmKelas").textContent = siswa.kelas;
        document.getElementById("confirmMapel").textContent = `${info.mapel} - ${info.jenis}`; document.getElementById("confirmWaktu").textContent = `${jadwal.mulai||'-'} - ${jadwal.selesai||'-'}`;
...
async function continueExam() {
    ...
    currentUser = pendingUser; currentUjian = pendingUjian; waktuSelesai = pendingWaktuSelesai; minimalMenit = pendingUjian.min || 0; idSesi = pendingSesiAktif.idSesi; waktuMulaiServer = new Date(pendingSesiAktif.waktuMulai);
    try { const s = localStorage.getItem(`skor_${idSesi}`); window.skorPerSoal = s ? JSON.parse(s) : {}; } catch (e) { window.skorPerSoal = {}; }
    document.getElementById("resumeScreen").style.display = "none"; document.getElementById("examScreen").style.display = "block";
    document.getElementById("namaDisplay").innerText = `${currentUser.nama} | ${currentUser.kelas}`; document.getElementById("infoDisplay").innerText = `${currentUjian.mapel} - ${currentUjian.jenis}`;
    await ambilSoal(currentUser.jenjang, currentUjian.mapel, currentUjian.jenis); await loadJawabanDariFirebase();
    mulaiTimer(); renderNavigator(); showFullscreenPrompt(); updateTombolSelesai(); setInterval(updateTombolSelesai, 1000);
}

async function startExam() {
    if (!pendingUser || !pendingUjian) { cancelConfirm(); return; }
    currentUser = pendingUser; currentUjian = pendingUjian; waktuSelesai = pendingWaktuSelesai; minimalMenit = pendingUjian.min || 0; waktuMulaiServer = new Date();
    idSesi = 'SES-' + Date.now() + '-' + Math.random().toString(36).substr(2,9);
    window.skorPerSoal = {}; localStorage.removeItem(`skor_${idSesi}`);
    if (window.db) window.Firebase.addDoc(window.Firebase.collection(window.db,'sesi_ujian'), { idSesi, username: currentUser.username, nis: currentUser.nis, nama: currentUser.nama, jenjang: currentUser.jenjang, kelas: currentUser.kelas, mapel: currentUjian.mapel, jenisUjian: currentUjian.jenis, waktuMulai: new Date().toISOString(), status: 'Aktif', token: document.getElementById("tokenInput").value, totalSkorSementara: 0 });
...
async function ambilSoal(j, m, js) {
    ...
    let mentah = [];
    for (let i = 1; i < rows.length; i++) if (String(rows[i][1]).trim() === String(j).trim() && String(rows[i][2]).trim() === String(m).trim() && String(rows[i][3]).trim() === String(js).trim() && rows[i][13] === "Aktif") mentah.push({ id: rows[i][0], tipe: rows[i][4], pertanyaan: rows[i][5], pilihan: [rows[i][6],rows[i][7],rows[i][8],rows[i][9],rows[i][10]].filter(p=>p), kunci: rows[i][11], bobot: parseFloat(rows[i][12])||1, gambar: rows[i][14], grupSoal: rows[i][15] });
    if (!mentah.length) { document.getElementById("soalContainer").innerHTML = "<p>Belum ada soal.</p>"; return; }
    dataSoal = acakSoalDenganGrup(mentah); renderSoal(0);
}
...
async function simpanJawabanKeFirebase(idSoal, jawaban, skor) {
    ...
    try {
        ...
        skorPerSoal[idSoal] = skor; localStorage.setItem(`skor_${idSesi}`, JSON.stringify(skorPerSoal));
...
// ==================== DRAG & DROP ====================
function initDragDropJodoh() { document.querySelectorAll('.matching-item-right[draggable="true"]').forEach(i => i.addEventListener('dragstart', e => e.dataTransfer.setData('text/plain', e.target.dataset.huruf))); document.querySelectorAll('.matching-target').forEach(t => { t.addEventListener('dragover', e => e.preventDefault()); t.addEventListener('drop', e => { e.preventDefault(); const key = t.dataset.key, huruf = e.dataTransfer.getData('text/plain'); if (!key || !huruf) return; if (!window.currentMatchingJawaban) window.currentMatchingJawaban = {}; if (window.currentMatchingJawaban[key]) { showError('Sudah terisi!'); return; } if (Object.values(window.currentMatchingJawaban).includes(huruf)) { showError('Sudah dipakai!'); return; } window.currentMatchingJawaban[key] = huruf; updateMatchingUIJodoh(); }); }); }
...
async function selesaiUjian() {
    ...
    let totalSkor = 0, jumlahBenar = 0, totalBobot = 0;
    ...
    dataSoal.forEach(s => { const b = s.bobot || 1; totalBobot += b; const j = jawabanLokal[s.id]; if (!j) return; if (s.tipe === "PG" && j === s.kunci) { totalSkor += b; jumlahBenar++; } else if (s.tipe === "Isian") { if (String(j).toLowerCase().trim() === String(s.kunci).toLowerCase().trim()) { totalSkor += b; jumlahBenar++; } } });
    if (totalSkor > totalBobot) totalSkor = totalBobot;
    const persen = totalBobot ? (totalSkor/totalBobot)*100 : 0;
    if (document.exitFullscreen) document.exitFullscreen(); document.getElementById("freezeOverlay").style.display = "none";
    if (window.db) {
        try { await window.Firebase.addDoc(window.Firebase.collection(window.db,'nilai_akhir'), { idSesi, username: currentUser.username, nis: currentUser.nis, nama: currentUser.nama, kelas: currentUser.kelas, mapel: currentUjian.mapel, jenisUjian: currentUjian.jenis, totalSkor, jumlahBenar, jumlahSoal: dataSoal.length, totalBobot, persentase: persen.toFixed(1)+'%', timestamp: new Date().toISOString() }); } catch(e){}
        try { const snap = await window.Firebase.getDocs(window.Firebase.query(window.Firebase.collection(window.db,'sesi_ujian'), window.Firebase.where('idSesi','==',idSesi))); snap.forEach(d => window.Firebase.updateDoc(d.ref, { status: 'Selesai', waktuSelesai: new Date().toISOString() })); } catch(e){}
    }
    localStorage.removeItem(`jawaban_${idSesi}`); localStorage.removeItem(`skor_${idSesi}`);
    showModal({ iconType: "success", title: "🎉 Ujian Selesai!", message: "", buttons: [{ text: "Tutup", type: "success", onClick: () => location.reload() }] });
    setTimeout(() => { document.querySelector(".modal-message").innerHTML = `<div style="text-align:center;"><div style="font-size:48px;font-weight:800;color:#1E3A8A;">${totalSkor}/${totalBobot}</div><div>Total Skor</div><div style="display:flex;justify-content:center;gap:20px;margin-top:16px;"><div><span style="color:#22C55E;">✅ ${jumlahBenar}</span> Benar</div><div><span style="color:#EF4444;">❌ ${dataSoal.length-jumlahBenar}</span> Salah</div><div>📊 ${persen.toFixed(1)}%</div></div></div>`; }, 50);
}
