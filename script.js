    ...
    try{
        ...
        if(window.db){
            try{
                ...
                const q=window.Firebase.query(coll,window.Firebase.where('username','==',siswa.username),window.Firebase.where('status','==','Aktif'),window.Firebase.where('mapel','==',tokenInfo.mapel));
                ...
                if(!snapshot.empty){
                    ...
                    const sisaDetik=Math.max(Math.floor((wS-n)/1000),0),sisaMenit=Math.floor(sisaDetik/60);
...
function cancelResume(){if(!confirm("Yakin ingin memulai ujian baru? Sesi sebelumnya akan dihapus."))return;if(pendingSesiAktif&&window.db){const coll=window.Firebase.collection(window.db,'sesi_ujian');window.Firebase.addDoc(coll,{...pendingSesiAktif,status:'Dibatalkan'});}document.getElementById("resumeScreen").style.display="none";document.getElementById("confirmScreen").style.display="block";document.getElementById("confirmNIS").textContent=pendingUser.nis||'-';document.getElementById("confirmNama").textContent=pendingUser.nama||'-';document.getElementById("confirmKelas").textContent=pendingUser.kelas||'-';document.getElementById("confirmMapel").textContent=`${pendingUjian.mapel} - ${pendingUjian.jenis}`;document.getElementById("confirmWaktu").textContent=`${pendingUjian.mulai} - ${pendingUjian.selesai}`}
async function continueExam(){if(!pendingUser||!pendingUjian||!pendingSesiAktif){showError("Data sesi tidak valid.");cancelResume();return}currentUser=pendingUser;currentUjian=pendingUjian;waktuSelesai=pendingWaktuSelesai;minimalMenit=pendingUjian.min||0;idSesi=pendingSesiAktif.idSesi;waktuMulaiServer=new Date(pendingSesiAktif.waktuMulai);totalPenalti=pendingSesiAktif.pelanggaran||0;document.getElementById("resumeScreen").style.display="none";document.getElementById("examScreen").style.display="block";document.getElementById("namaDisplay").innerText=`${currentUser.nama||'N/A'} | ${currentUser.kelas||'N/A'}`;document.getElementById("infoDisplay").innerText=`${currentUjian.mapel||'N/A'} - ${currentUjian.jenis||'N/A'}`;await ambilSoal(currentUser.jenjang,currentUjian.mapel,currentUjian.jenis);await loadJawabanDariFirebase();mulaiTimer();renderNavigator();showFullscreenPrompt();updateTombolSelesai();setInterval(updateTombolSelesai,1000);showSuccess(`Selamat datang kembali, ${currentUser.nama||'Siswa'}!`)}
async function startExam(){if(!pendingUser||!pendingUjian){showError("Data tidak valid.");cancelConfirm();return}currentUser=pendingUser;currentUjian=pendingUjian;waktuSelesai=pendingWaktuSelesai;minimalMenit=pendingUjian.min||0;waktuMulaiServer=new Date();idSesi='SES-' + Date.now() + '-' + Math.random().toString(36).substr(2,9);if(window.db){try{const coll=window.Firebase.collection(window.db,'sesi_ujian');await window.Firebase.addDoc(coll,{idSesi,username:currentUser.username,nis:currentUser.nis,nama:currentUser.nama,jenjang:currentUser.jenjang,kelas:currentUser.kelas,mapel:currentUjian.mapel,jenisUjian:currentUjian.jenis,waktuMulai:new Date().toISOString(),status:'Aktif',token:document.getElementById("tokenInput").value.trim()});}catch(e){console.error('Gagal simpan sesi:',e);}}document.getElementById("confirmScreen").style.display="none";document.getElementById("examScreen").style.display="block";document.getElementById("namaDisplay").innerText=`${currentUser.nama||'N/A'} | ${currentUser.kelas||'N/A'}`;document.getElementById("infoDisplay").innerText=`${currentUjian.mapel||'N/A'} - ${currentUjian.jenis||'N/A'}`;await ambilSoal(currentUser.jenjang,currentUjian.mapel,currentUjian.jenis);mulaiTimer();renderNavigator();showFullscreenPrompt();updateTombolSelesai();setInterval(updateTombolSelesai,1000);showSuccess(`Selamat datang, ${currentUser.nama||'Siswa'}!`)}

// ==================== LOAD JAWABAN DARI FIREBASE ====================
async function loadJawabanDariFirebase(){if(!window.db||!idSesi||!currentUser)return;try{const coll=window.Firebase.collection(window.db,'jawaban_siswa');const q=window.Firebase.query(coll,window.Firebase.where('idSesi','==',idSesi));const snapshot=await window.Firebase.getDocs(q);let loaded=0;snapshot.forEach(doc=>{const data=doc.data();jawabanLokal[data.idSoal]=data.jawaban;loaded++;});if(loaded>0){renderNavigator();showToast(`${loaded} jawaban dimuat`,'info',2000);}}catch(e){console.error('Gagal load jawaban:',e);}}
...
async function selesaiUjian(){
    ...
    if(window.db&&idSesi){
        try{
            ...
            const snapshot=await window.Firebase.getDocs(q);
            snapshot.forEach(async(doc)=>{await window.Firebase.addDoc(coll,{...doc.data(),status:'Selesai',waktuSelesai:new Date().toISOString()});});
        }catch(e){}
    }
